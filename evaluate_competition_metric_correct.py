...
def evaluate_with_competition_metric(
    model_path,
    val_data_root="/home/114078/projectA_work/dataset_correct/val",
    conf_threshold=0.25
):
    """
    使用比賽官方評分標準評估模型
    ...
    if final_score > 0.85:
        ...
        gap = (0.85 - final_score) * 100
        print(f"⚠ 需要改進（差距: {gap:.2f}%）")
        print(f"\n建議:")
        print(f"  1. 降低 confidence threshold (當前: {conf_threshold})")
        print(f"  2. 調整 IoU threshold")
        print(f"  3. 考慮使用更大的模型 (YOLOv8m)")

    # 保存結果
    results_file = Path("/home/114078/projectA_work/competition_evaluation_correct.txt")
    with open(results_file, 'w', encoding='utf-8') as f:
        f.write("="*80 + "\n")
        f.write("比賽官方評分結果 - 修正版模型\n")
        f.write("="*80 + "\n\n")
...
if __name__ == "__main__":
    # 使用修正版模型
    model_path = "/home/114078/projectA_work/models/best_model_correct.pt"

    if not Path(model_path).exists():
        print(f"錯誤：找不到模型 {model_path}")
        print("\n可用的模型:")
        models_dir = Path("/home/114078/projectA_work/models")
    ...
    print(f"\n關鍵結論:")
    print(f"  Weighted F1: {final_score*100:.2f}%")
    print(f"  比賽要求: 85%")
    
    if final_score >= 0.85:
        print(f"  狀態: ✓ 已達標！")
    elif final_score >= 0.80:
        print(f"  狀態: ⚠ 接近達標，微調參數即可")
    else:
        print(f"  狀態: ✗ 需要改進")
